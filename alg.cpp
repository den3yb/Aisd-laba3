...
namespace algoritms {
    ...
    void cycle_random_quick(){
        ...
        for (int i = 0; i < 7; i++) {
            stats s;
            s = one_random_quick(lenght[i], i);
            cout << "Для quicksort случайного массива длиной " << lenght[i] << " колличество сравнений: " << s.comparison_count << ", а колличество копий: " << s.copy_count << "\n";
        }   
    }
    ...
    void cycle_random_buble() {
        int lenght[] = { 1000,3000,5000,10000,25000,50000,100000 };
        for (int i = 0; i < 7; i++) {
            stats s;
            s = one_random_bubble(lenght[i], i);
            cout << "Для bubble случайного массива длиной " << lenght[i] << " колличество сравнений: " << s.comparison_count << ", а колличество копий: " << s.copy_count << "\n";
        }
    }
